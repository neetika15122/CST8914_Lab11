...
//   };
// });
// JavaScript for Accordion with ARIA and keyboard support
...
accordionBtns.forEach((accordion) => {
  accordion.onclick = function () {
    ...
    // Toggle the max-height for the accordion content visibility
    if (content.style.maxHeight) {
  ...
  // Handle keyboard interactions (Enter, Spacebar, Arrow keys)
  accordion.addEventListener("keydown", function (e) {
    if (e.key === "Enter" || e.key === " " || e.key === "Spacebar") {
      e.preventDefault();
      this.click();
    } else if (e.key === "ArrowDown" || e.key === "ArrowUp") {
      const accordionsArray = Array.from(accordionBtns);
      const currentIndex = accordionsArray.indexOf(this);
      let nextIndex = currentIndex;

      if (e.key === "ArrowDown") {
        nextIndex = (currentIndex + 1) % accordionsArray.length;
      } else if (e.key === "ArrowUp") {
        nextIndex = (currentIndex - 1 + accordionsArray.length) % accordionsArray.length;
      }

      // Focus the next accordion button
      accordionsArray[nextIndex].focus();
    }
  });
});
